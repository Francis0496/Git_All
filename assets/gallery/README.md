# Gallery Asset Guide

The website gallery now includes these evidence images:

- academic-program-graduates-01.jpeg
- academic-program-graduates-02.jpeg
- ai-emerging-tech-session-01.jpeg
- community-impact-group.jpeg
- demo-day-audience.jpeg
- graduation-award-interview.jpeg
- graduation-awards.jpeg
- hackathon-competition-poster.jpeg
- information-sessions2.jpeg
- ipam-monlanding-hackathon.jpeg
- knesst-activities.jpeg
- knesst-demo-landing-portrait.jpeg
- sierra-edutech-training.jpeg
- spotlight-newsletter.jpeg
- technical-training-lab-01.jpeg
- tutor.jpeg
- workshop-facilitation-01.jpeg
- workshop-facilitation-02.jpeg
- writing-centre.jpeg

Recommended image treatment:

- Use real photos or scanned recognition/publication images only.
- Export as JPG, JPEG, or WebP, around 1600px wide for large images.
- Keep each file under 350 KB where possible.
- Prefer clear, bright evidence photos over abstract graphics.
- Avoid uploading private student data, phone numbers, signatures, or IDs unless permission is explicit.

The site preloads each listed image and only switches the visual tile from generated placeholder art to the real image when the file successfully loads.
